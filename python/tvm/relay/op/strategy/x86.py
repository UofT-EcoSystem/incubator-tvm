...
def conv2d_strategy_cpu(attrs, inputs, out_type, target):
    ...
    if groups == 1:
        if layout == "NCHW":
            ...
            # TODO: we should have better way to handle this
            ...
            if len(kernel.shape) == 4:
                ...
                is_winograd_applicable = "float" in data.dtype and \
                                         ...
                                         dilation_h == 1 and dilation_w == 1 and \
                                         64 < co < 512
...
def conv2d_transpose_strategy_cpu(attrs, inputs, out_type, target):
    ...
    if layout == "NCHW":
        ...
        strategy.add_implementation(wrap_compute_conv2d_transpose(topi.nn.conv2d_transpose_nhwc),
                                    wrap_topi_schedule(ansor.auto_schedule_topi),
                                    name='ansor')
    else:
...
def conv2d_winograd_without_weight_transfrom_get_kernel_size(attrs, kernel):
