...
def tensor_core_meet_condition(search_policy, state, stage_id):
    pass
...
def tensor_core_apply(search_policy, state, stage_id):
    ret = []
    state = ansor.loop_state.State(state, search_policy.cur_task.compute_dag)

    A, B, C = search_policy.cur_task.compute_dag.ops
